...
# model = SentenceTransformer('intfloat/e5-large-v2') # Example of a different model
...
def precompute_embeddings(chunks):
    ...
    chunk_contents = ['passage: ' + chunk["summary"] for chunk in chunks]
...
def compute_metrics(retrieved_ids, relevant_chunks):
    """
    Compute various metrics for the retrieved chunks.
    ...
    def compute_ndcg():
        # Create a relevance score map for retrieved items
        ...
        # This is the DCG score for a "perfectly" sorted list
...
def process_query(chunks, query_text, ground_truth_ids, top_n=5):
    ...
    similarities = [
        (chunk["rule_id"], cosine_similarity([query_embedding], [chunk["embedding"]])[0][0])
        for chunk in chunks
    ]
    ...
    # Compute metrics by comparing retrieved IDs to the "correct" ground truth IDs
    metrics = compute_metrics(retrieved_ids, ground_truth_ids)
    
    # Add retrieved chunk details for inspection
...
# --- Main Workflow ---
chunk_file_path = "/Users/abhishekprasad/Documents/RAG/Document Editor/data/style_guide_chunks.json"
ground_truth_file_path = "/Users/abhishekprasad/Documents/RAG/Document Editor/data/style_guide_rules.json"
K_VALUE = 5 # This is the 'K' in Precision@K, Recall@K. How many results to retrieve.
...
# 3. Run all test queries and collect metrics
all_metrics = []
print(f"\nRunning {len(ground_truth)} test queries...")

for i, test in enumerate(ground_truth):
    query = test["rule_id"] 
    ground_truth_ids = [test["rule_id"]]
    
    # Process the query and score it
...
# Get the average of all the numeric metric columns
